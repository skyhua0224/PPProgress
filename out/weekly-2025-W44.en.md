# Weekly Report (2025-10-27 to now)

## Dev progress (ProjectPrinting + submodules)

# Dev progress — ProjectPrinting + submodules (since 2025-10-27)

Modules scanned: ProjectPrinting, admin, frontend, backend
Total commits: 3
Total files touched: 1155
Total lines +/-: +174163 / -0

## Per-module summary
- ProjectPrinting (.): commits=1, files=128, +51536/-0
- admin (admin): commits=1, files=440, +67813/-0
- frontend (frontend): commits=0, files=0, +0/-0
- backend (backend): commits=1, files=587, +54814/-0

## Heat by top-level directory (per module)
- ProjectPrinting:
  - docs: 43623
  - docs-old: 2155
  - file_list.txt: 874
  - COUPON_CAMPAIGN_CREATE_MOCKUP_V2.md: 645
  - coupon_system_analysis.md: 469
  - COUPON_CAMPAIGN_MOCKUP.md: 465
  - analysis: 393
  - 修复前后对比详解.md: 390
  - TASKS.md: 340
  - 修复总结_FulfillmentName一致性问题.md: 307
  - 完整修复总结.md: 292
  - coupon-campaign-detail-mockup.html: 247
  - .github: 234
  - 修复检查清单.md: 198
  - coupon-campaign-create-mockup.html: 194
- admin:
  - pnpm-lock.yaml: 11095
  - src/views/marketing: 8345
  - packages: 6136
  - src/views/system: 4565
  - src/views/store: 3269
  - src/service/api: 3228
  - src/views/platform: 3060
  - src/views/printing-center: 2885
  - src/views/order: 2837
  - src/views/hardware: 2499
  - src/store/modules: 2287
  - src/layouts/modules: 2012
  - src/locales/langs: 1883
  - src/typings/api.d.ts: 1727
  - src/typings/app.d.ts: 1213
- backend:
  - src/main/java: 51126
  - api.md: 1517
  - src/main/resources: 1122
  - docs: 264
  - mvnw: 259
  - pom.xml: 190
  - mvnw.cmd: 149
  - .gitignore: 47
  - src/test/java: 47
  - .vscode: 38
  - HELP.md: 27
  - .github: 21
  - .spring-api-helper.json: 5
  - .cloudbase: 1
  - .mvn: 1

## Commit subjects (up to 10 per module)
- ProjectPrinting:
  - [undefined] [admin] 自动合并[93719f5a: feat(printing-center): 添加搜索面板、统计数据与相关类型/接口]\n
- admin:
  - [undefined] feat(printing-center): 添加搜索面板、统计数据与相关类型/接口
- backend:
  - [undefined] feat: 添加打印中心统计接口与实现
